...
หลังติดตั้ง migration 012 แล้ว ให้รัน `supabase/migrations/013_match_dates_and_public_brackets.sql` เพื่อเปิดใช้วันแข่งขันที่แก้ไขได้และหน้าสาธารณะ `/brackets` ไฟล์นี้รันซ้ำได้ เติมวันให้สายเดิมโดยเริ่ม 25/08/2026 และข้ามเสาร์–อาทิตย์ โดยไม่ลบรายชื่อหรือสุ่มสายใหม่

หากผู้สมัครเจอ `duplicate key value violates unique constraint "players_public_id_unique"` ตอนกดบันทึก แปลว่า sequence ของ Public Player ID ตามหลังรายชื่อจริงอยู่ ให้รัน `supabase/migrations/008_collision_free_player_ids.sql` ใน SQL Editor ไฟล์นี้รันซ้ำได้ เปลี่ยนวิธีออก ID ให้ข้ามเลขที่มีคนใช้แล้วและดึง sequence กลับมาตรงกับรายชื่อ ไม่ต้องลบหรือแก้ผู้เล่นเดิม

ถ้าลบผู้เล่นท้ายรายชื่อออกแล้วอยากให้คนถัดไปได้เลขต่อจากคนที่เหลืออยู่จริง (ลบ `DT-987`, `DT-988`, `DT-989` แล้วคนถัดไปควรได้เลขต่อจากคนสุดท้ายที่เหลือ ไม่ใช่ `DT-990`) ให้รัน `supabase/migrations/009_public_id_follows_roster.sql` ไฟล์นี้รันซ้ำได้และรันแทน 008 ได้เลยถ้ายังไม่เคยลง 008 ข้อควรระวัง: ถ้ารัน 003 ซ้ำเมื่อไหร่ default ของ `public_id` จะกลับไปเป็นแบบเดิม ต้องรัน 009 ซ้ำทุกครั้งหลังรัน 003
...
สำหรับการแบ่งสายแยกชาย/หญิงเป็นสองทัวร์นาเมนต์ย่อย ให้รัน `supabase/migrations/010_gender_divisions.sql` หลังไฟล์ 009 ไฟล์นี้รันซ้ำได้และไม่ลบรายชื่อเดิม จากนั้นในหน้า `/admin` แต่ละแถวผู้เล่นจะมีปุ่ม ชาย/หญิง ให้กำหนดเพศ ระบบจะจับสายให้ก็ต่อเมื่อผู้เล่นทุกคนถูกกำหนดเพศครบ และแต่ละสายมีอย่างน้อย 2 คน หลังกดสุ่ม สายการแข่งขันจะแยกเป็นสองแท็บ ชาย/หญิง ผู้เล่นชายกับหญิงจะไม่เจอกัน หน้าผู้เล่นไม่เปลี่ยน แต่ละคนยังเห็นเฉพาะสายของตัวเอง

สำหรับสายคู่ผสม (ชาย 1 + หญิง 1 ต่อฝั่ง) ให้รัน `supabase/migrations/012_mixed_doubles_division.sql` หลังไฟล์ 011 ไฟล์นี้รันซ้ำได้ ทีมจะถูกจับจากตำแหน่งที่สุ่มได้ในสายของแต่ละคน คนที่ 1 ของสายชายคู่กับคนที่ 1 ของสายหญิง แล้วทีม 1 เจอทีม 2 ทีม 3 เจอทีม 4 ไปตามลำดับ หลังรันแล้วต้องกด **สุ่มคู่แข่งขันใหม่** หนึ่งครั้ง สายคู่ผสมจึงจะถูกสร้าง และจะมีแท็บ คู่ผสม เพิ่มขึ้นทั้งหน้าแอดมินและหน้าผู้เล่น
